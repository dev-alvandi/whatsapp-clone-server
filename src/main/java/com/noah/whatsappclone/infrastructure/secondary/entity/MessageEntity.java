...
import com.noah.whatsappclone.messaging.domain.message.aggregate.MessageBuilder;
import com.noah.whatsappclone.messaging.domain.message.valueObject.*;
...
public class MessageEntity extends AbstractAuditingEntity<Long> {
    ...
    public static MessageEntity from(Message message) {
        ...
        if (message.getContent()
            ...
            messageEntityBuilder.text(message.getContent()
                    .text());
        } else {
        ...
        UserEntity user =
                UserEntityBuilder.userEntity()
                        .publicId(message.getPublicId()
                                .value())
                        .build();
        messageEntityBuilder
                .type(message.getContent()
                        .type())
                .publicId(message.getPublicId()
                        .value())
    ...
    public static Message toDomain(MessageEntity messageEntity) {
        MessageBuilder messageBuilder = MessageBuilder.message()
                ...
                .sender(new UserPublicId(messageEntity.getSender()
                        .getPublicId()))
                .publicId(new MessagePublicId(messageEntity.getPublicId()));

        if (messageEntity.getType()
                .equals(MessageType.TEXT)) {
            MessageContent content = new MessageContent(messageEntity.getText(),
                    MessageType.TEXT,
                    null);
            messageBuilder.content(content);
        } else {
            MessageMediaContent messageMediaContent = new
                    MessageMediaContent(messageEntity.getContentBinary()
                    .getFile(),
                    messageEntity.getContentBinary()
                            .getFileContentType());
            MessageContent content = new MessageContent(messageEntity.getText(),
    ...
    public boolean equals(Object o) {
        ...
        return Objects.equals(getPublicId(),
                that.getPublicId()) && Objects.equals(getSendTime(),
                that.getSendTime()) && Objects.equals(getText(),
                that.getText()) && getType() == that.getType() && getSendState() == that.getSendState();
    }

    @Override
    public int hashCode() {
        int result = Objects.hashCode(getPublicId());
        result = 31 * result + Objects.hashCode(getSendTime());
        result = 31 * result + Objects.hashCode(getText());
        result = 31 * result + Objects.hashCode(getType());
        result = 31 * result + Objects.hashCode(getSendState());
        return result;
    }
}
